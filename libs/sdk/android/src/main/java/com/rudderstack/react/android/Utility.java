    ...
    static RudderOption convertReadableMapToOptions(ReadableMap readableMap) {
        ...
        try {
            ...
            for (String key : optionsMap.keySet()) {
                switch (key) {
                    case EXTERNAL_ID:
                        setExternalId(EXTERNAL_ID, readableMap, options);
                        break;
                    case EXTERNAL_IDS:
                        setExternalId(EXTERNAL_IDS, readableMap, options);
                        break;
                    case INTEGRATIONS:
                        setIntegrations(optionsMap, options);
                        break;
                    default:
                        setCustomContext(key, optionsMap, options);
                        break;
                }
            }
        } catch (Exception e) {
